...
exec pnpm exec concurrently --kill-others-on-fail -n serve,proxy -c magenta,cyan \
  ...
  "MCP_GUARDIAN_DB_PATH='$MCP_GUARDIAN_DB_PATH' DASHBOARD_ENABLED=false GUARDIAN_DASHBOARD_SPA=false GUARDIAN_WS_ENABLED=false METRICS_ENABLED=true node dist/cli.js proxy --config '$CONFIG' --policy '$POLICY' --blocking-mode audit"
